...
func (h *CourseHandler) CreateCourseForm(context *gin.Context) {
	context.HTML(http.StatusOK, "create-course.html", nil)
}

func (h *CourseHandler) CreateCourse(context *gin.Context) {
	var courseForm forms.CourseForm
	courseForm.Name = context.PostForm("name")
	courseForm.Description = context.PostForm("description")
	...
	context.Redirect(http.StatusSeeOther, "/courses/")
}
...
func (h *CourseHandler) GetAllCourses(context *gin.Context) {
	...
	context.HTML(http.StatusSeeOther, "courses.html", courseList)
}
