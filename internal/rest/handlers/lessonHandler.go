...
func (lh *LessonHandler) GetAllLessons(context *gin.Context) {
	...
	logger.GetLogger().Info("Course get successfully")
	context.HTML(http.StatusOK, "lessons.html", lessonList)
}

func (lh *LessonHandler) CreateLessonForm(context *gin.Context) {
	context.HTML(http.StatusOK, "create-lessons.html", nil)
}

func (lh *LessonHandler) CreateLesson(context *gin.Context) {
	var lessonForm forms.LessonForm
	lessonForm.Name = context.PostForm("name")
	lessonForm.Description = context.PostForm("description")
	lessonForm.VideoUrl = context.PostForm("videoUrl")
	...
	context.Redirect(http.StatusSeeOther, "/lessons/")
}
